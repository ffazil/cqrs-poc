...
import com.tracebucket.x.cqrs.annotation.ReplyTo;
...
public class ApplicationServiceAutoConfiguration implements ApplicationListener<ContextRefreshedEvent>,
	...
	public static final ReflectionUtils.MethodFilter CONSUMER_METHOD_FILTER = new ReflectionUtils.MethodFilter() {
		...
		public boolean matches(Method method) {
			return null != AnnotationUtils.findAnnotation(method, CommandHandler.class);
		}
	};

	private static final Logger LOG = LoggerFactory.getLogger(ApplicationServiceAutoConfiguration.class);
	...
	public ApplicationServiceAutoConfiguration() {
	...
	public void onApplicationEvent(ContextRefreshedEvent ev) {
		...
		for (String beanName : ctx.getBeanDefinitionNames()) {
			...
			if (null == AnnotationUtils.findAnnotation(type, ApplicationService.class)) {
	...
	public void wireBean(final Object bean, final Set<Method> methods) {
		...
		Consumer consumer;
		Observable reactor;
		CommandHandler selectorAnno;
		ReplyTo replyToAnno;
		reactor.event.selector.Selector selector;

		for (final Method method : methods) {
			//scanAnnotation method
			selectorAnno = AnnotationUtils.findAnnotation(method, CommandHandler.class);
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Attaching Consumer to Reactor[" + reactor + "] using CommandHandler[" + selector + "]");
			}
	...
	private Observable fetchObservable(CommandHandler selectorAnno, Object bean) {
		return expression(selectorAnno.reactor(), bean);
	}

	private Object parseSelector(CommandHandler selector, Object bean, Method method) {
	...
	private reactor.event.selector.Selector fetchSelector(CommandHandler selectorAnno, Object bean, Method method) {
	...
	private static class DirectFieldAccessPropertyAccessor implements PropertyAccessor {
		...
		@Override
		public void write(EvaluationContext context, Object target, String name, Object newValue) throws
				AccessException {
			Field fld = findField(target, name);
			try {
